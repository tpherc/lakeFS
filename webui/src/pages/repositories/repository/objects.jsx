...
const UploadCandidate = ({
    ...
    return (
        <div className={`upload-item upload-item-${state ? state.status : 'pending'}`}>
            ...
            <div className="upload-status">{statusIndicator}</div>
        </div>
    );
};
                ...
                <Modal.Body>
                    ...
                    {uploadState.error && !isAbortedError(uploadState.error, abortController) && (
                        <AlertError
                            ...
                            onDismiss={() => setUploadState((prev) => ({ ...prev, error: null }))}
                        />
                    )}
                </Modal.Body>
                <Modal.Footer>
    ...
    if (show && response) {
        return (
            <Alert variant="warning" onClose={closeAndRemember} dismissible>
                ...
                <a href="https://docs.lakefs.io/howto/garbage-collection/gc/" target="_blank" rel="noreferrer">
                    Learn More
                </a>
                .
            </Alert>
